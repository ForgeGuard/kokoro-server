...
async def _find_file(
    ...
    Security:
        ``filename`` may come from user-controlled routes (e.g. the web player
        and the audio download endpoint), so absolute paths and ``..`` traversal
        are rejected — a resolved path that escapes its search directory is
        skipped rather than served.
    """
    for path in search_paths:
        base = os.path.realpath(path)
        full_path = os.path.realpath(os.path.join(base, filename))
        # Reject anything that resolves outside the search directory.
