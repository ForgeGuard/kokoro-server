...
class ModelManager:
    ...
    def __init__(self, config: Optional[ModelConfig] = None):
        ...
        # Guards against unloading the model out from under an in-flight
        # generation: generate() registers itself, unload() waits for the
        # count to reach zero before freeing the backend.
...
2. Or set environment variable in docker-compose:
   DOWNLOAD_MODEL=true
""")
            raise RuntimeError(f"Model files not found: {e}") from e
        except Exception as e:
    ...
    async def generate(self, *args, **kwargs):
        ...
        # Register this generation and capture a local reference to the backend
        # so a concurrent unload() (which waits for _inflight to hit zero) can
        # neither free the model mid-stream nor swap the reference under us.
    ...
    async def unload(self) -> None:
        ...
        Waits for any in-flight generation to finish before freeing the backend
        so a running request never dereferences a freed model.
        """
        async with self._cond:
